/**
 * SIMPLE REAL TRADING SYSTEM - JavaScript Version
 * Direct execution without TypeScript compilation
 */

// Load environment variables from .env file
require('dotenv').config();

const { alpacaClient } = require('./dist/services/alpaca/alpaca-client.cjs');
    ...
    try {
      ...
      while (this.tradesPlaced < this.config.maxTrades) {
        ...
        try {
          ...
          // Filter for today's options (0-DTE)
          ...
          if (todayOptions.length > 0) {
            // Simple strategy: find OTM call with highest volume
