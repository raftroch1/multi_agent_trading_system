# CLAUDE CODING GUIDELINES - SPY TRADING SYSTEM

## 🚫 MANDATORY APPROVAL REQUIRED
**DO NOT CREATE OR MODIFY FILES WITHOUT FOLLOWING THESE GUIDELINES**

## 📁 PROJECT ARCHITECTURE (Clean Architecture)

```
spy-trading-system-enhanced/
├── src/
│   ├── controllers/           # Orchestration layer
│   │   ├── trading-controller.ts
│   │   ├── signal-controller.ts
│   │   └── position-controller.ts
│   ├── agents/               # Market analysis (KEEP ALL)
│   │   ├── multi-agent-market-analysis.ts
│   │   ├── order-flow-agent.ts
│   │   ├── volume-profile-enhanced.ts
│   │   └── [all existing agents]
│   ├── services/             # External integrations
│   │   ├── alpaca/
│   │   │   ├── alpaca-client.ts
│   │   │   ├── alpaca-paper-trading.ts
│   │   │   └── alpaca-streaming.ts
│   │   └── market-data/
│   │       ├── data-feed.ts
│   │       └── options-feed.ts
│   ├── strategies/           # Trading strategies
│   │   ├── naked-options/
│   │   │   ├── naked-options-executor.ts
│   │   │   └── strike-selector.ts
│   │   └── position-management/
│   │       ├── dynamic-profit-manager.ts
│   │       ├── enhanced-strike-selector.ts
│   │       └── position-orchestrator.ts
│   ├── utils/                # Shared utilities
│   │   ├── technical-indicators.ts
│   │   ├── greeks-engine.ts
│   │   └── transaction-cost-engine.ts
│   ├── types/                # Type definitions
│   │   ├── index.ts          # Export all types
│   │   ├── trading.types.ts
│   │   ├── market.types.ts
│   │   └── alpaca.types.ts
│   └── config/               # Configuration
│       ├── app.config.ts
│       ├── alpaca.config.ts
│       └── trading.config.ts
├── tests/                    # Test files
├── docs/                     # Documentation
├── scripts/                  # Build/deployment scripts
├── index.ts                  # Main entry point
├── package.json
└── tsconfig.json
```

## 🎯 PROJECT SCOPE
- **TARGET**: 0-DTE SPY naked options (calls/puts only)
- **GOAL**: $300/day on $25K account (1.2% return)
- **STRATEGY**: Multi-agent consensus + simple execution
- **BROKER**: Alpaca paper trading integration

## 📋 CODING STANDARDS

### 1. BEFORE CREATING FILES:
- [ ] Check if similar functionality exists in existing files
- [ ] Verify file placement in correct folder
- [ ] Ensure dependency direction is correct (no circular imports)
- [ ] Document purpose in Memory Keeper

### 2. FILE NAMING:
- Files: kebab-case (trading-controller.ts)
- Classes: PascalCase (TradingController)
- Methods: camelCase (executeBuyCall)
- Constants: UPPER_SNAKE_CASE (MAX_POSITIONS)
- Interfaces: PascalCase (ITradingSignal)

### 3. DEPENDENCY RULES:
- Controllers can depend on Services and Agents
- Services can depend on Utils and Types
- Agents can depend on Utils and Types
- Utils can ONLY depend on Types
- NEVER: Utils → Controllers, Agents → Services

### 4. EXISTING FILES TO PRESERVE:
✅ **KEEP ALL AGENTS**: Your sophisticated multi-agent system
✅ **KEEP POSITION MANAGERS**: DynamicProfitManager, EnhancedStrikeSelector
✅ **KEEP UTILITIES**: TechnicalIndicators, GreeksEngine
✅ **KEEP TYPES**: All existing type definitions

### 5. FILES TO MOVE:
- Move alpaca.ts → src/services/alpaca/alpaca-client.ts
- Move existing agents → src/agents/
- Move position managers → src/strategies/position-management/
- Move utils → src/utils/
- Move types → src/types/

## 🚫 FORBIDDEN ACTIONS:
- ❌ Creating duplicate functionality
- ❌ Modifying existing working agents
- ❌ Breaking existing imports
- ❌ Creating files without proper placement
- ❌ Ignoring error handling patterns
- ❌ Hardcoding configuration values
- ❌ MASS FILE OPERATIONS: Never move multiple files at once - move individually
- ❌ BULK RENAME OPERATIONS: Never use wildcards for critical file moves
- ❌ AUTOMATED REORGANIZATION: Always verify each file move individually
- ❌ **MOCK DATA**: NEVER create mock/simulated market data
- ❌ **PAPER TRADING SIMULATION**: Use real Alpaca paper trading API only
- ❌ **FAKE MARKET DATA**: Connect directly to live market data feeds

## ✅ REQUIRED APPROVALS:
Before any file creation/modification:
1. **Memory Keeper Check**: Document purpose and scope
2. **Architecture Review**: Verify correct placement
3. **Dependency Analysis**: Check import direction
4. **Existing Code Review**: Ensure no duplication
5. **BACKUP VERIFICATION**: Confirm backups exist before file operations
6. **INDIVIDUAL FILE OPERATIONS**: Move files one at a time, verifying each move
7. **REAL DATA VERIFICATION**: Ensure all data comes from live Alpaca/market APIs

## 📈 REAL INTEGRATION REQUIREMENTS:
- ✅ **LIVE MARKET DATA**: Must connect to real-time SPY market data via Alpaca
- ✅ **REAL OPTIONS CHAINS**: Must fetch live options chains from market APIs
- ✅ **PAPER TRADING**: Must use actual Alpaca paper trading API for order execution
- ✅ **LIVE GREEKS**: Must calculate real Greeks from live market data
- ✅ **REAL VOLUME**: Must use actual volume and open interest data

## 🎯 INTEGRATION APPROACH:
1. **Direct Alpaca Integration**: Connect existing alpaca-client.ts to live APIs
2. **Real Market Data**: Use Alpaca market data APIs for SPY price and volume
3. **Live Options Chains**: Connect to real options data sources
4. **Actual Order Execution**: Execute real paper trades through Alpaca
5. **Real-time Monitoring**: Track actual positions and P&L from Alpaca

## 🔧 SAFE FILE OPERATIONS:
- Use `cp` (copy) before `mv` (move) to verify operation
- Test file operations on non-critical files first
- Verify folder structure exists before moving files
- Check file permissions after operations
- Always keep original files until operation is verified

## 🎯 SUCCESS METRICS:
- All existing functionality preserved
- Clean folder structure achieved
- Working Alpaca integration
- No circular dependencies
- Comprehensive error handling

---
**Remember: Your existing multi-agent system is sophisticated and excellent. Focus on integration, not replacement.**